...
1- Run the [encirclement_launch.py](crazy_encirclement/launch/encirclement_launch.py) file:
...
2 - For safety, first open a terminal and run the node that sends a landing command to all the drones:
...
3- Now, the nodes that send the commands to the drones has to be run. For that, open one terminal for each drone you want to fly, and run: 
...
4- After running one node for each drone, they will NOT take off. You have to click on the terminal where each node is running and press ```Enter```

5- After all of them take off, in another terminal, run the the node that sends the drone a flag to start the trajectory:
